...
vars = {i: model.addVar(vtype="B", name=f"x_{i}") for i in graph.nodes} # integer version
# vars = {i: model.addVar(vtype="C", ub = 1, name=f"x_{i}") for i in graph.nodes} # continuous version
...
for e in graph.edges:
    model.addCons(
        vars[e[0]] + vars[e[1]] >= 1,
        name=f"constraint for edge {e}",
    )

# add objective function
model.setObjective(scip.quicksum(vars.values()), sense="minimize")
...
print(f"   {[i for i in graph.nodes if model.getVal(vars[i]) > 0.9]}")

# draw graph with matching
for i in graph.nodes: 
    if model.getVal(vars[i]) > 0.9:
